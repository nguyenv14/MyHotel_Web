        ...
        <div class="card">
            <div class="card-body">
                <div style="display: flex;justify-content: space-between">
                    <div class="card-title col-sm-9">Thư Viện Video Khách Sạn {{ $hotel->hotel_name }}
                    </div>
                    <div class="col-sm-3">
                    </div>
                </div>

                <form action="{{ URL::to('admin/hotel/manager/insert-gallery') }}"
                    method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="hotel_id" value="{{ $hotel->hotel_id }}">
                    <input type="hidden" name="type" value="2">
                    <div class="mb-3">
                        <label for="formFile" class="form-label">Thêm Video Vào Thư Viện </label>
                        <input class="form-control" type="file" name="file[]" id="formFile" accept="video/*" multiple>
                    </div>
                    <div>
        ...
        $(document).ready(function() {
            ...
            $(document).on('blur', '.update_gallery_hotel_name', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        message_toastr("success", "Tên Video Đã Được Cập Nhật !");
                         load_gallery(notePage);
                    },
                    error: function(data) {
                        alert("Nhân Ơi Fix Bug Huhu :<");
            ...
            /* Cập Nhật Nội Dung Video Gallery */
            $(document).on('blur', '.edit_gallery_hotel_content', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        message_toastr("success", "Nội Dung Video Đã Được Cập Nhật !");
                         load_gallery(notePage);
                    },
                    error: function(data) {
                        alert("Nhân Ơi Fix Bug Huhu :<");
            ...
            $(document).on('click', '.delete_gallery_hotel', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == 'true') {
                            message_toastr("success", "Video Đã Được Xóa !");
                        } 
                         load_gallery(notePage);
            ...
            $(document).on('change', '.up_load_file', function() {
                ...
                form_data.append("file", document.getElementById('up_load_file' + gallery_hotel_id).files[0]);
                ...
                $.ajax({
                    ...
                    processData: false,
                    success: function(data) {
                        message_toastr("success", "Cập Nhật Video Thành Công !");
                         load_gallery(notePage);
                    },
                    error: function(data) {
                        alert("Nhân Ơi Fix Bug Huhu :<");
            ...
            $('#formFile').change(function() {
                ...
                if (files.length > 1) {
                    error += 'Chỉ Được Up 1 Video';

                } else if (files.length == '') {
                    error += 'Vui lòng chọn Video';

                } else if (files.size > 100000000) {
                    error += 'Video Không Được Lớn Hơn 100Mb';
                }
